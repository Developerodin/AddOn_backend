...
yarnConeSchema.post('save', async function (doc) {
  if (doc.returnedToVendorAt != null) {
    if (doc.boxId) {
      try {
        const { syncBoxLtRemainingFromCones } = await import(
          '../../services/yarnManagement/yarnBoxLtRemaining.sync.js'
        );
        await syncBoxLtRemainingFromCones(doc.boxId);
      } catch (boxError) {
  ...
  const shouldSyncStInventory = hasStorage && isAvailable && hasWeight;
  ...
  try {
    ...
    if (doc.boxId) {
      try {
        ...
        await syncBoxLtRemainingFromCones(doc.boxId, { coneIssueDate: doc.createdAt || new Date() });
      } catch (boxError) {
        console.error(`[YarnCone] Error resetting box ${doc.boxId}:`, boxError.message);
      }
    }
