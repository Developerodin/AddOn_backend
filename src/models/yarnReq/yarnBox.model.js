...
const yarnBoxSchema = mongoose.Schema(
  {
    ...
    /** Canonical YarnCatalog reference (backfilled by migration; used for sync). */
    yarnCatalogId: {
      type: mongoose.Schema.Types.ObjectId,
      ref: 'YarnCatalog',
      required: false,
...
// Auto-generate barcode from _id before saving; resolve yarnCatalogId / sync yarnName from catalog
yarnBoxSchema.pre('save', async function (next) {
  if (!this.barcode) {
    ...
    if (this.yarnName && this.isModified('yarnName')) {
      ...
      if (!catalog) {
        catalog = await YarnCatalog.findOne({
          yarnName: {
            $regex: new RegExp(`^${this.yarnName.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i'),
          },
          status: { $ne: 'deleted' },
        })
          .select('_id')
